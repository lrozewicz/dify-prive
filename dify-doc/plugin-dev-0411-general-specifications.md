...
* `options` (list\[[provider\_config\_option](#providerconfigoption-object)]): Options, only used when type is `select`
...
* `provider` (string): Model provider name containing plugin\_id, in the form of `langgenius/openai/openai`.
