...
* CONSOLE\_CORS\_ALLOW\_ORIGINS: Console CORS cross-domain policy, default is `*`, that is, all domains can access.
* WEB\_API\_CORS\_ALLOW\_ORIGINS: WebAPP CORS cross-domain policy, default is `*`, that is, all domains can access.
...
* UPLOAD\_FILE\_SIZE\_LIMIT: Upload file size limit, default 15M.

* UPLOAD\_FILE\_BATCH\_LIMIT: The maximum number of files that can be uploaded at a time, default 5.
